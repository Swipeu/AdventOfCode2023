...
function getPreviousInSequence(sequence: number[]): number{
    const diffSequence = getDiffSequence(sequence);
    if(diffSequence.every(n => n === 0)){
        return sequence[0];
    }

    return sequence[0] - getPreviousInSequence(diffSequence);
}
...
file.split(/\r?\n/).forEach((line, y) => {
    ...
    numberArray.push(getPreviousInSequence(originalSequence));
